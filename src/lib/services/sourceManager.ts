...
export class SourceManager {
  ...
  async updateSource(
    ...
    try {
      ...
      if (updates) {
        ...
        if (
          updates.tags &&
          JSON.stringify(updates.tags) !== JSON.stringify(source.metadata.tags)
        ) {
      ...
      try {
        ...
        if (fresh_content.success) {
          ...
          const current_hash = this.generateContentHash(
            fresh_content.content.text,
          );
          if (
      ...
      const result: SourceUpdateResult = {
        ...
        changes: has_changes
          ? {
              ...
              content:
                source.metadata.contentHash !==
                original_source.metadata.contentHash,
              metadata:
                updates?.category !== original_source.metadata.category ||
                JSON.stringify(updates?.tags) !==
                  JSON.stringify(original_source.metadata.tags),
            }
          : undefined,
        updatedAt: new Date(),
      };
  ...
  async validateSources(sourceIds?: string[]): Promise<{
    ...
    const sources_to_validate = sourceIds
      ? await Promise.all(
          sourceIds.map((id) => storageService.getSource(id)).filter(Boolean),
        )
  ...
  async detectDuplicates(): Promise<{
    ...
    try {
      ...
      for (const source of sources) {
        ...
        const similar_sources = sources.filter(
          (s) =>
            s.id !== source.id &&
            !processed.has(s.id) &&
            this.calculateSimilarity(source, s) > 0.8,
        );
      ...
      const summary = {
        ...
        duplicateSources: duplicates.reduce(
          (sum, d) => sum + d.duplicates.length + 1,
          0,
        ),
      };
  ...
  async getSourceStatistics(): Promise<{
    ...
    const stats = {
      ...
      recentlyUpdated: sources.filter((s) => s.lastChecked > one_day_ago)
        .length,
    };
  ...
  async integrateWithKnowledgeSystem(
    ...
    try {
      const integration_result =
        await knowledgeSystemIntegration.integrateContent(
          source,
          processedModule,
        );
  ...
  > {
    ...
    try {
      const recommendations =
        await knowledgeSystemIntegration.getContentRecommendations(
          userId,
          completedContent,
          max_recommendations,
        );
      ...
      for (const rec of recommendations) {
        ...
        const source = sources.find((s) =>
          s.usage.generatedModules.includes(rec.contentId),
        );
