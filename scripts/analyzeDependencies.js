...
class DependencyAnalyzer {
    ...
    addDependency(importPath) {
        ...
        if (!importPath.startsWith('.') && !importPath.startsWith('/') && !this.isBuiltinModule(packageName)) {
    ...
    isBuiltinModule(moduleName) {
        const builtinModules = [
            ...
            'buffer', 'process', 'child_process', 'cluster', 'dgram', 'dns',
            'domain', 'http', 'https', 'net', 'querystring', 'readline',
            'repl', 'string_decoder', 'tls', 'tty', 'vm', 'zlib'
        ];
    ...
    generateReport() {
        ...
        console.log(`Used dependencies: ${usedCount} (${Math.round(usedCount / totalDependencies * 100)}%)`);
        console.log(`Unused dependencies: ${unusedCount} (${Math.round(unusedCount / totalDependencies * 100)}%)`);
        console.log('');
    ...
    categorizeDependencies(dependencies) {
        ...
        dependencies.forEach(dep => {
            ...
            } else if (dep.includes('test') || dep.includes('vitest') || dep.includes('jest') || dep.includes('jsdom')) {
    ...
    generateRecommendations(unusedDependencies, categories) {
        ...
        if (unusedDependencies.length > 0) {
            console.log(`1. Remove ${unusedDependencies.length} unused dependencies to reduce bundle size`);
        }
        ...
        if (categories['Utilities'].length > 3) {
            console.log('3. Review utility dependencies - some might be replaceable with native functions');
        }
