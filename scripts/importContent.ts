...
async function main() {
  ...
  const server = (await import('../src/lib/mcp/web-content/web-content-mcp-server.js')).webContentMcpServer;
  server.initialize();
  ...
  if (contentId) {
    transformResult = await server.executeTool('transformToInteractive', {
      contentId,
